...
async fn main() -> Result<()> {
        ...
        Ok(response) => {
            ...
            if let Some(snapshots) = response.tickers {
                ...
                for (index, snapshot) in snapshots.iter().take(10).enumerate() {
                    ...
                    if let (Some(change), Some(change_perc)) = (snapshot.todays_change, snapshot.todays_change_perc) {
                        let direction = if change >= 0.0 { "📈" } else { "📉" };
                        println!("   {} Change: ${:.2} ({:.2}%)", direction, change, change_perc);
                    }
                    
                    // Show last trade info if available
                    if let Some(last_trade) = &snapshot.last_trade {
                        println!("   💰 Last Trade: ${:.2} (size: {})", last_trade.price, last_trade.size);
                    }
    ...
    println!("   • Snapshot data includes: price changes, bid/ask spreads, volume, and daily ranges");
...
fn format_snapshot_summary(snapshot: &polygon_io::stocks::TickerSnapshot) -> String {
    // Try to get the most recent price from various sources
    let current_price = snapshot.last_trade.as_ref()
