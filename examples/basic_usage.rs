...
async fn main() -> Result<()> {
    ...
    let api_key = std::env::var("POLYGON_API_KEY")
        .expect("Please set POLYGON_API_KEY environment variable");
    ...
    match stocks.market_status().await {
        Ok(status) => {
            ...
            if let Some(exchanges) = &status.exchanges {
                println!("  • Exchanges: {}", serde_json::to_string_pretty(exchanges).unwrap_or_default());
            }
        }
