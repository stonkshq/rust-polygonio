...
fn calculate_basic_stats(bars: &[polygonio::stocks::AggregateBar], timeframe: &str) {
    ...
    let high_prices: Vec<f64> = bars.iter().filter_map(|bar| bar.high).collect();

    let low_prices: Vec<f64> = bars.iter().filter_map(|bar| bar.low).collect();

    let volumes: Vec<f64> = bars.iter().filter_map(|bar| bar.volume).collect();
